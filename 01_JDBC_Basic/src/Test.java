	...
	public static void main(String[] args) {
		...
		String sql = "SELECT TNO, TNAME, TDATE FROM TEST";
			...
			try {
				...
				// 1) JDBC드라이버 등록
				// ojdbc6.jar 파일이 등록 안됐거나, 오타가 있을 경우 => ClassNotFoundException 예외 발생
				Class.forName("oracle.jdbc.driver.OracleDriver");
				System.out.println("ORACLE JDBC 드라이버가 로드됐습니다.");
				
				// 2) Connect 객체 생성(url(DB위치), username, password)
				conn = DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:xe","JDBC","JDBC");
				
				// 3) Statement 객체 생성(sql문 실행에 필요한 객체)
				stmt = conn.createStatement(); //conn이 담고 있는 객체에 
				
				// 4),5) sql문을 전다하면서 실행 후 결과(조회된 데이터)를 ResultSet 객체에 받기
				rset = stmt.executeQuery(sql); //stmt객체를 이용하여 쿼리문을 실행
				
				// 6) ResultSet에 담겨있는 데이터들을 하나씩 추출
				//	  ResultSet의 next() 메서드는 조회된 레코드가 있는지 확인해서 있으면 ture, 없으면 false 반환
				while(rset.next()) //다음에 데이터가 있는지 체크, 있으면 true 없으면 false 있을때까지 처리
				{
					//rest에 담겨있는 칼럼을 변수에 담기
					tno = rset.getInt("tno");
